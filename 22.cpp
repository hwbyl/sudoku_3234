...
bool check_horizontal_right(int a)
{
	...
	for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			if(board[a][i]!=0&&board[a][j]!=0&&i!=j&&board[a][i]==board[a][j]){
				return false;
				break;
			}
			else if(board[a][i]==0||board[a][j]==0){
				return false;
				break;
			}
			else continue;
...
bool check_vertical_right(int b)
{
    ...
    for(int i=0;i<4;i++){
		for(int j=0;j<4;j++){
			if(board[i][b]!=0&&board[j][b]!=0&&i!=j&&board[i][b]==board[j][b]){
				return false;
				break;
			}
			else if(board[i][b]==0||board[j][b]==0){
				return false;
				break;
			}
			else continue;
	...
	for(int i=0;i<2;i++){
		for(int j=0;j<2;j++){
			for(int l=0;l<2;l++){
				for(int k=0;k<2;k++){
					if(i!=l || j!=k){
						if(board[a1+i][b1+j]!=0 && board[a1+l][b1+k]!=0 && board[a1+i][b1+j]==board[a1+l][b1+k]){
							return true;
							break;
						}
						else continue;
...
bool check_block_right(int a,int b)
{
	...
	for(int i=0;i<2;i++){
		for(int j=0;j<2;j++){
			for(int l=0;l<2;l++){
				for(int k=0;k<2;k++){
					if(board[a1+i][b1+j]==0||board[a1+l][b1+k]==0){
						return false;
						break;
					}
					else if(i!=l || j!=k){
						if(board[a1+i][b1+j]==board[a1+l][b1+k]){
							return false;
							break;
						}
					else continue;
